...
export default function Terminal(screenTextEngine: {
	...
	function onInput() {
		...
		oldText = textarea.value; if (change)
	...
	window.addEventListener("keypress", (e) => {
		if (textarea.readOnly === true || document.activeElement?.id !== "textarea") {
			textarea.readOnly = false;
			textarea.focus();
	...
	let lastSelection = 0; document.addEventListener("selectionchange", () => {
