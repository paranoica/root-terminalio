...
export default function WebGL() {
	loadAssists((assists) => {
		...
		const camera = new THREE.PerspectiveCamera(
			50, sizes.width / sizes.height, 0.1, 100
		);
		...
		let mousedown: { x: number; y: number } | null = null; function checkIfTouch(event: PointerEvent) {
		...
		const computerParallax = { x: 0, y: 0 }; canvas.addEventListener(
			"pointermove", (event) => {
				checkIfTouch(event);
				if (mousedown) {
					computerParallax.x +=
		...
		window.addEventListener(
			"resize", () => {
				...
				updateCanvasSize(sizes.width, sizes.height); sizes.portraitOffset = valMap(
		...
		const plane = new THREE.Mesh(planelikeGeometry, new THREE.MeshBasicMaterial({ color: "blue" }));
		...
		const tick = () => {
			...
			camera.position.x = computerParallax.x * valMap(scroll, [0, 1], [0.2, 5]) * 0.1 + camera.position.x * 0.9;
			camera.position.y = computerParallax.y * valMap(scroll, [0, 1], [0.2, 1.5]) * 0.1 + camera.position.y * 0.9;
