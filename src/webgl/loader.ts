...
function loadAssists(callback: (assists: Assists) => any) {
	...
	manager.onProgress = function (url, itemsLoaded, itemsTotal) {
		if (!loadingItemsDOM || !loadingBarDOM)
			return;
	...
	const fontLoader = new FontLoader(manager); fontLoader.load("/fonts/public-pixel.json", (font) => {
	...
	const textureLoader = new THREE.TextureLoader(manager); textureLoader.load("/textures/bake-quality-5.jpg", (tex) => {
		tex.flipY = false;
	...
	const cubeTextureLoader = new THREE.CubeTextureLoader(manager); cubeTextureLoader.load(
		[
	...
	const gltfLoader = new GLTFLoader(manager); gltfLoader.load("/models/Commodore710_33.5.glb", (gltf) => {
