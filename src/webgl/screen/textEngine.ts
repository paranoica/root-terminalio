...
export default function ScreenTextEngine(
	...
	const caret = new THREE.Mesh(
		...
		textMaterial
	); caret.position.z = -0.1; rootGroup.add(caret);
	...
	function updateCaret(pos?: number) {
		...
		if (pos !== undefined) {
			...
			charPos.y = -(charNextLoc.y + h2Font.leading * Math.floor((pos + terminalPromptOffset) / charsPerLine));
	...
	function generateGeometry(props: {
		...
		props.updateCharNextLoc = props.updateCharNextLoc !== undefined ? props.updateCharNextLoc : true;

		const strLen = (props.font.width + props.font.tracking) * props.str.length;
		const strWrapLen = props.isWord ? (props.font.width + props.font.tracking) * (props.str.length - 1) : props.font.width * props.str.length;
		...
		if (props.updateCharNextLoc)
			...
			textGeometry.translate(0, -props.font.height, -0.001);
	...
	let terminalPromptOffset = 0; function placeText(str: string) {
		const charWidth = h2Font.width + h2Font.tracking;
		const charsPerLine = Math.floor(screenWidth / charWidth);
	...
	function updateCharPos(
		...
		for (let i = 0; i < inputBuffer.length; i++) {
			...
			const y = -(charNextLoc.y + h2Font.leading * Math.floor((i + terminalPromptOffset) / charsPerLine));
	...
	function placeImage(val: string) {
		...
		if (Number.isNaN(aspectRatio))
			throw new Error(`Error for image at: '${url}'. Image must have aspect ratio like this: '/path/to/image?aspect=1.5'`);
